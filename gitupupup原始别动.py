...
# ---------------- CONFIG ----------------
# 把你的 GitHub 凭证写在这里（按你要求：写在代码里，不显示在界面）
GITHUB_USERNAME = "1372601383-web"
GITHUB_TOKEN = ""  # 填 token（如果不填，脚本将使用本地 git 凭证）
GITHUB_REPO = "sun-of-beach-mother-father"
GITHUB_BRANCH = "main"

# 本地仓库根目录：默认脚本所在目录（可改成绝对路径）
...
def windows_long_path(path):
    """在 Windows 上尝试为超长路径返回 \\?\\ 前缀版本，其他平台返回原始路径"""
    if os.name == "nt":
        if path.startswith("\\\\?\\"):
            return path
        # must be absolute
...
def copy_tree_with_retry(src, dst, logger):
    """
    递归复制 src 到 dst，尽量兼容超长路径与特殊字符。
    返回 (success_count, fail_list)
    """
    fail_list = []
    success_count = 0
    for root, dirs, files in os.walk(src):
        ...
        for f in files:
            ...
            try:
                ...
                # try Windows long path variant if on Windows
                try:
                    sp = windows_long_path(src_path)
                    dp = windows_long_path(dst_path)
                    # ensure dst parent exists (using normal path)
...
class UploadWorker(QtCore.QRunnable):
    ...
    def run(self):
        try:
            ...
            # 1) check target exists -> block and inform user (per requirement)
            if os.path.exists(dest):
                self.log(f"目标仓库根目录已存在同名文件夹：{folder_name}。上传被阻止，请重命名本地文件夹后重试。")
                self.signals.finished.emit(False, "目标文件夹已存在于仓库根目录，已阻止上传。请重命名后重试。")
                return

            # 2) count images in source
            image_paths = []
            ...
            # 3) copy files to repo_root (dest)
            ...
            # optional embed token temporarily if provided
            original_remote = None
            if self.token:
                try:
                    p = subprocess.run(["git", "remote", "get-url", "origin"], cwd=self.repo_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                    original_remote = p.stdout.strip()
                    if original_remote.startswith("https://"):
                        parts = original_remote.split("https://", 1)[1]
                        new_remote = f"https://{self.token}@{parts}"
                        subprocess.run(["git", "remote", "set-url", "origin", new_remote], cwd=self.repo_root)
                        self.log("已临时把 token 嵌入远程 URL（上传完将尝试还原）。")
                except Exception as e:
                    self.log(f"嵌入 token 时出错：{e}")

            # run git add
            r = subprocess.run(["git", "add", folder_name], cwd=self.repo_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if r.returncode != 0:
                self.log(f"git add 返回错误：{r.stderr.strip()}")
                # still continue to attempt commit/push? we'll abort
                self.signals.finished.emit(False, f"git add 失败: {r.stderr.strip()}")
                return

            commit_msg = f"Add {folder_name}"
            r = subprocess.run(["git", "commit", "-m", commit_msg], cwd=self.repo_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            # commit may be non-zero if nothing to commit; just log output
            self.log(r.stdout.strip() + ("\n" + r.stderr.strip() if r.stderr.strip() else ""))

            # git pull --rebase
            self.log(f"git pull --rebase origin {self.branch}")
            r = subprocess.run(["git", "pull", "origin", self.branch, "--rebase"], cwd=self.repo_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            self.log(r.stdout.strip() + ("\n" + r.stderr.strip() if r.stderr.strip() else ""))

            # git push with live stderr parsing
            self.log("开始 git push（解析 stderr 以提取上传速度）")
            push_cmd = ["git", "push", "origin", self.branch]
            proc = subprocess.Popen(push_cmd, cwd=self.repo_root, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, bufsize=1)

            # We'll parse stderr lines; also update progress by scanning git index? Git doesn't give per-file finished count easily.
            # We'll approximate progress using number of images and increment as we detect "Writing objects" or similar lines.
            # Better: after push completes, we generate excel using local file list.
            last_speed = "-"
            while True:
                ...
                if line:
                    ...
                    if ('/s' in line) or ('KiB/s' in line) or ('MiB/s' in line) or ('B/s' in line):
                        # try find token that endswith '/s'
                        tokens = [tok for tok in line.split() if tok.endswith("/s")]
                        if tokens:
                            last_speed = tokens[-1]
                            self.signals.speed.emit(last_speed)
                    # some git progress lines contain "Writing objects:  33% (1351/4062)"
                    if "Writing objects:" in line and "(" in line and "/" in line:
                        # try to extract numbers in parentheses like (1351/4062)
                        try:
                            ...
                            if "/" in part:
                                ...
                                # map git object counts to image counts approximately:
                                # We'll set progress to min(total_images, done * total_images // max(1, total))
            ...
            if rc != 0:
                ...
                # restore remote if modified
                if original_remote:
                    try:
                        subprocess.run(["git", "remote", "set-url", "origin", original_remote], cwd=self.repo_root)
                        self.log("已还原远程 URL（移除 token）。")
                    except:
                        pass
            ...
            # walk dest tree and collect images (preserve order)
            rows = []
            for root, _, files in os.walk(dest):
                for fname in files:
                    if os.path.splitext(fname)[1].lower() in IMAGE_EXT:
                        ...
                        self.signals.progress.emit(done, total_images)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def dropEvent(self, event):
        ...
        if count == 0:
            ...
            return
        # check repo root existing same folder
